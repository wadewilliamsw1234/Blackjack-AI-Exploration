...
def select_action():
	...
	with torch.no_grad():
		scores = model(FloatTensor(np.array([state_machine.state()]))).data
	mask = ByteTensor(1 - state_machine.mask())
	best_action = (scores.masked_fill_(mask.bool(), -16)).max(-1)[1]
...
def optimize_model():
	...
	if not_terminal.sum() > 0: 
		model.eval()
		with torch.no_grad():
			not_terminal_states = torch.cat([s for s in batch[2] if s is not None])
		masks = ByteTensor(np.array([1 - state_machine.mask_for(s) for s in not_terminal_states.data.numpy()]))
		V_s[not_terminal] = (model(not_terminal_states).data.masked_fill_(masks.bool(), -16)).max(1)[0]
		model.train()
